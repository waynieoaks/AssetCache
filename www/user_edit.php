...
$sql_usernames = "SELECT LOWER( p.username ) AS username FROM users p WHERE p.userid != $recordid";
...
$sql_emails = "SELECT LOWER(p.email) AS email FROM users p WHERE p.userid != $recordid";
    ...
    <div class="row">
        <div class="col-md-6">
            <form method="post" id="update-form" data-parsley-validate="" action="">
                ...
                <a href="user_show.php?id=<?php echo $recordid; ?>" class="btn btn-outline-secondary float-end">Cancel</a>
            </form>
        </div>
    </div>
</div>
